...
In the matrix, there seems to be a strong correlation with: unemployment-turnout, turnout-white, bachelor's degree and higher income, and percent white-turnout.

In the matrix there seems to be a weak correlation with: turnout and bachelors degree higher, income-median_age, income_pct-white, turnout-pcts_bachelors_degree.

In the scatterpolots, there seems to be a linear relationship with unemployment, % white, income, and bachelor's higher

There seems to be no relationship with population, and median age. 
...
The model overall is useful because the adjusted R-square shows that 80% of the variables in voter turnout are explained by the model. The p-value is very very small and the f-statistic shows significance. The residuals are also significantly small. 

The model does predict turnout for some things in the model, but not all. The variables of income, median_age and percent white are significant. However, the rest like population, unemployed, and bachelor's degree aren't significant at all.
...
Model 4 has an adjusted R-squared of 0.8023 works slightly better than the full model that was 0.8022. My model 3 and 2 were significantly worse with an r-squared of 0.6099876 and 0.5238997. None of the alternative models significantly work better than the full model. Model 4 appears slightly better, but the difference is too small to be great. That slight increase in Model 2 could also fall in the SD of the full model. So its not a great difference.


```{r}
# Model 2
model2 <- lm(turnout ~ pct_bachelors_higher + income + population, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ income + pct_unemployed + population, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ pct_white + median_age + income, data = md_turnout)
summary(model4) 
...
I would use this analysis to show readers that voter turnout in Maryland is strongly tied to race, age and income.
It specifically shows people that have higher income, white and of older age tend to be more likely to show up to the ballot box. I would write a voter participation story that highlight lower turnout or lack of engagement by comparing parts of a county that have high income, white and higher age, compared to diverse and younger communities. It could show resource disparities or just simple difference of interests within people.

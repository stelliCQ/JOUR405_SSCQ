...
The relationship between FARMS % and math test scores seems to have no relationship if you look at the line of best fit. But, if you look at the data points it seems that the majority accumulates on the left side of the graph.
...
The model is not worth using since we have a lot of variation between the minimum and maximum values. The model currently tells us that there is no relationship between FARMS percentage and math scores in Montgomery County.

Notes- stella 
The P-value is not real since its above 0.05. 
The adjusted R-squared doesn't account for anything because it seems to be explained by something else.
The residuals is how far away they are from the model. residuals smaller if better results.
...
The largest differences between the predicted results and actual math scores are that they are far off from what the model predicted when it comes to the residuals. This means that the model is not accurate, or there are really big existing outliers. 
...
The school board member is incorrect in his assessment because that is not the case for the majority of the schools in the county. Some schools in the minority have that occurring, but its not a true correlation. 

It would influence my pitch because first his statement isn't entirely correct. I would pitch to my editor that what he's using that statement in a proxy-way. To clarify that to my editor, the fewer students on FARMS due better on tests is due to that the student body have higher income levels than other schools, meaning they have more resources. The underlying reality is that poverty is what he's really referencing. 

...
```{r}
 hot_murders <- hot_murders |> mutate(murd_diff = temperature - murders)
#this was not useful for this LOL.
```

```{r}
hot_murders |> summarise(correlation = cor(murders, temperature, method = "pearson"))
```
...
hot_murders |> ggplot() + geom_point(aes(x=temperature, y=murders)) + 
...
The scatterplot shows that there's a moderate positive correlation between
increase in temperatures and an increase in murders. The calculation is 0.5, which

...
```{r}
soccerfoul <- read.csv("https://raw.githubusercontent.com/dwillis/NCAAWomensSoccerData/refs/heads/main/data/ncaa_womens_soccer_matchstats_2024.csv")
```
...
```{r}
soccerfoul |> summarise(correlation = cor(fouls, goal_diff, method = "pearson"))
#Not related at all.
```
...
soccerfoul |> ggplot() + geom_point(aes(x=goal_diff, y = fouls)) +
  geom_smooth(aes(x=goal_diff, y=fouls), methods= "lm")

```

### Task 6: Describe the results of both the correlation calculation and the scatterplot.

The scatterplot shows NO correlation at all between goal differences and fouls in
NCAA soccer teams. It looks like a huge block with no possible way to draw a line.
The correlation is 0.018. 

...
No, I don't think the model is useful because 2/3 independent variables are not significant individually. Furthermore, the adjusted R-square says that the model can only be predicted by 58.28%, which is a majority, but has a lot of leg room for things to change.  
...
The points scored has the most impact on wins. It seems that takeways does not have an impact on wins at all. I could also drop the yards gained since it seems correlated as well to points scored.  
...
*This is based on a new model from the yards allowed, giveaways, and points scored shown in class*

I would use the model to compare the points scored, giveaways and yards allowed would end up with wins in 
the gameby a conference. 

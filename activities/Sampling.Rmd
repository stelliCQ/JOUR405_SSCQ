...
```{r}
sample_100 |> summarise(mean(age), sd(age))
...
population |> summarise(mean(age), sd(age))
...
sample_100 |> summarise(mean(age), sd(age))
sample_50 |> summarise(mean(age), sd(age))
sample_10 |> summarise(mean(age), sd(age))
...
The results are similar for what the sample size of 50 and 100 and the actual population, while sample 10 is
a little more skewed. The results changed only when the sample size became smaller, meaning that 
as long as its a representative sample it doesn't need to be that big numbers wise.
